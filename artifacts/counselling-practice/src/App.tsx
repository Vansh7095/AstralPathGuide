    ...
    <div className="grain min-h-[100dvh] overflow-x-hidden bg-[hsl(var(--background))]">
      <header className="sticky top-0 z-40 border-b border-[hsl(var(--border)/.7)] bg-[hsl(var(--background)/.9)] backdrop-blur-lg">
        <div className="page-wrap flex h-[76px] items-center justify-between">
          <Link href="/" className="focus-ring group flex min-w-0 items-center gap-3" data-testid="link-logo">
            ...
            <span className="min-w-0"><span className="serif block max-w-[190px] truncate text-lg font-semibold leading-none text-[hsl(var(--foreground))]">{practiceName}</span><span className="mt-1 block text-[9px] font-bold uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">Counselling practice</span></span>
          </Link>
          <div className="hidden min-w-0 flex-1 items-center justify-center xl:flex">
            <nav className="flex items-center gap-5" aria-label="Primary navigation">
              {navItems.map((item) => <Link key={item.href} href={item.href} className={`focus-ring relative whitespace-nowrap py-2 text-[13px] font-semibold transition after:absolute after:bottom-0 after:left-0 after:h-px after:bg-[hsl(var(--accent))] after:transition-all ${location === item.href ? 'text-[hsl(var(--primary))] after:w-full' : 'text-[hsl(var(--muted-foreground))] after:w-0 hover:text-[hsl(var(--primary))] hover:after:w-full'}`} data-testid={`link-nav-${item.label.toLowerCase().replaceAll(' ', '-')}`}>{item.label}</Link>)}
            </nav>
          </div>
          <div className="hidden shrink-0 items-center gap-2 xl:flex">
            {isSignedIn ? <><ButtonLink href={workspace} secondary className="px-4 py-2.5 text-xs" testId="link-header-workspace">Workspace</ButtonLink><button type="button" onClick={() => void signOut()} className="focus-ring rounded-full px-2 py-2 text-xs font-bold text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--primary))]" data-testid="button-header-sign-out">Sign out</button></> : <><Link href="/sign-in" className="focus-ring rounded-full px-3 py-2 text-xs font-bold text-[hsl(var(--primary))] hover:bg-[hsl(var(--secondary))]" data-testid="link-header-login">Log in</Link><ButtonLink href="/sign-up" secondary className="px-4 py-2.5 text-xs" testId="link-header-sign-up">Sign up</ButtonLink></>}
            <ButtonLink href="/book" className="whitespace-nowrap px-4 py-2.5 text-xs" testId="link-header-book">Book a first step <ArrowUpRight size={14} /></ButtonLink>
          </div>
          <button type="button" aria-label={mobileOpen ? 'Close navigation' : 'Open navigation'} onClick={() => setMobileOpen((value) => !value)} className="focus-ring grid size-11 place-items-center rounded-full border border-[hsl(var(--border))] text-[hsl(var(--primary))] xl:hidden" data-testid="button-mobile-menu">{mobileOpen ? <X size={21} /> : <Menu size={21} />}</button>
        </div>
